      ...
      <div className="flex items-center gap-2">
        ...
        {isExpired && (
          <div className="relative">
            ...
            {showMusicPicker && (
              <div className="absolute top-12 right-0 w-56 bg-slate-900/90 backdrop-blur-2xl border border-white/20 rounded-2xl p-3 shadow-2xl text-white z-50 animate-in fade-in zoom-in-95">
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-xl text-xs transition-colors flex items-center justify-between ${
                      audioTrack === 'musicbox' ? 'bg-emerald-600 text-white font-bold' : 'hover:bg-white/10'
                    }`}
                  >
                    <span>Music Box</span>
                    {audioTrack === 'musicbox' && '✓'}
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-xl text-xs transition-colors flex items-center justify-between ${
                      audioTrack === 'acoustic' ? 'bg-emerald-600 text-white font-bold' : 'hover:bg-white/10'
                    }`}
                  >
                    <span>Acoustic</span>
                    {audioTrack === 'acoustic' && '✓'}
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-xl text-xs transition-colors flex items-center justify-between ${
                      audioTrack === 'party' ? 'bg-emerald-600 text-white font-bold' : 'hover:bg-white/10'
                    }`}
                  >
                    <span>Party</span>
                    {audioTrack === 'party' && '✓'}
