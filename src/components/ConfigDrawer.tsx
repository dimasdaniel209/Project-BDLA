    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative z-10 w-full max-w-xl bg-[#031d14]/95 text-white border-l border-emerald-500/30 shadow-2xl flex flex-col h-full overflow-hidden backdrop-blur-2xl">
        ...
        <div className="p-5 border-b border-emerald-500/20 flex items-center justify-between bg-emerald-950/40">
          <div>
            <h2 className="text-lg font-extrabold flex items-center gap-2 text-emerald-200">
              ⚙️ Pengaturan & Edit Kejutan
            </h2>
            <p className="text-emerald-300/70 text-xs">Sesuaikan nama, tanggal, pesan, foto, dan kode akses</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-emerald-950/60 border border-emerald-500/30 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-400"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-emerald-200 mb-1">
              ...
              <div className="bg-emerald-950/40 border border-emerald-500/30 rounded-2xl p-3.5 space-y-2">
                <label className="block text-xs font-bold text-emerald-300 flex items-center gap-1.5">
                  <Calendar className="w-4 h-4 text-emerald-400" /> Target Tanggal & Waktu Ulang Tahun
                </label>
                <input
                  type="datetime-local"
                  value={toLocalDatetimeInputValue(formData.birthDate)}
                  onChange={(e) => handleGeneralChange('birthDate', fromLocalDatetimeInputValue(e.target.value))}
                  className="w-full bg-emerald-900/40 border border-emerald-500/40 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-400 font-mono"
                />
                <div className="flex items-center justify-between gap-2 pt-1">
                  <span className="text-xs text-amber-300 font-medium flex items-center gap-1">
                    🕒 Waktu Terpilih: <strong className="text-white font-semibold">{formatIndonesianDateTime(formData.birthDate) || 'Belum diatur'}</strong>
                  </span>
                </div>
                <p className="text-[11px] text-emerald-300/60">
                  💡 Tips: Waktu di atas adalah waktu lokal yang otomatis sinkron dan akurat ke semua perangkat.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-emerald-500/20">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-emerald-950/60 border border-emerald-500/30 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-400 font-mono tracking-wider"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-emerald-200 mb-1">
              ...
              <div className="pt-2 border-t border-emerald-500/20">
                ...
                <textarea
                  ...
                  className="w-full bg-emerald-950/60 border border-emerald-500/30 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-emerald-400"
                />
              </div>

              <div className="pt-2 border-t border-emerald-500/20">
                <label className="block text-xs font-bold text-emerald-200 mb-1 flex items-center gap-1">
                  <MessageSquare className="w-3.5 h-3.5 text-amber-300" /> Pesan Spesial Utama (Isi Surat)
                </label>
                <textarea
                  rows={4}
              ...
              <div className="pt-2 border-t border-emerald-500/20 space-y-2">
                <label className="block text-xs font-bold text-emerald-200 mb-1 flex items-center gap-1">
                  <Music className="w-3.5 h-3.5 text-emerald-400" /> Musik Latar Default
                </label>
                <select
                ...
                {formData.audioTrackId === 'custom' && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-emerald-950/60 border border-emerald-500/30 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-400"
                    />
                  </div>
                )}
              ...
              <div className="pt-3 border-t border-emerald-500/20">
                <label className="block text-xs font-bold text-emerald-200 mb-1 flex items-center gap-1">
                  <Lock className="w-3.5 h-3.5 text-amber-400" /> PIN Admin (Pengunci Pengaturan)
                </label>
                <input
                  type="text"
          ...
          {activeSubTab === 'memories' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs text-emerald-300">Daftar Foto & Kenangan Spesial</span>
                <button
                  onClick={handleAddMemory}
                  className="bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-bold px-3 py-1.5 rounded-xl flex items-center gap-1 transition-colors"
                >
              ...
              <div className="space-y-3">
                {formData.memories.map((mem, index) => (
                  <div
                    key={mem.id}
                    className="bg-emerald-950/60 p-3 rounded-2xl border border-emerald-500/30 space-y-2 relative"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-emerald-300/60 hover:text-red-400 p-1"
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        ...
                        className="bg-emerald-900/50 border border-emerald-500/30 rounded-lg p-2 text-xs text-white"
                      />
                      <input
                        type="text"
                        value={mem.date}
                    ...
                    <input
                      ...
                      className="w-full bg-emerald-900/50 border border-emerald-500/30 rounded-lg p-2 text-xs text-white"
                    />
          ...
          {activeSubTab === 'vouchers' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs text-emerald-300">Voucher Hadiah Yang Bisa Diklaim</span>
                <button
              ...
              <div className="space-y-3">
                {formData.vouchers.map((v, idx) => (
                  <div
                    key={v.id}
                    className="bg-emerald-950/60 p-3 rounded-2xl border border-emerald-500/30 space-y-2"
                  >
                    <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      className="w-full bg-emerald-900/50 border border-emerald-500/30 rounded-lg p-2 text-xs text-amber-300 font-mono"
                    />
                  </div>
                ))}
              </div>
          ...
          {activeSubTab === 'encryption' && (
            <div className="space-y-5 animate-in fade-in duration-300">
              ...
              <div className="bg-black/60 p-4 rounded-2xl border border-emerald-500/20 space-y-2 text-xs text-emerald-200/80">
                <h5 className="font-bold text-amber-300 flex items-center gap-1.5">
                  💡 Cara Kerja Otomatis Saat Di-Host / Dibagikan:
                </h5>
                <ol className="list-decimal list-inside space-y-1.5 text-emerald-300/70 leading-relaxed">
                  <li>
        ...
        <div className="p-4 border-t border-emerald-500/20 bg-[#02150e] flex items-center justify-between gap-3">
          <button
            onClick={handleReset}
            className="text-emerald-300/70 hover:text-white text-xs flex items-center gap-1 px-3 py-2 rounded-xl hover:bg-white/5 transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-emerald-950/80 hover:bg-emerald-900 text-emerald-200 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors border border-emerald-500/30"
            >
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-extrabold px-4 py-2 rounded-xl flex items-center gap-1.5 shadow-lg active:scale-95 transition-transform"
            >
